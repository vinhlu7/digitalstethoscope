package com.example.digitalstethoscope.views;
...
public class TestCanvasView extends View {
    ...
    public void setPictureData(int screenWidth, int screenHeight,
            int[] rectCoord, int[] colors) {
        this.rectCoord = rectCoord;
        this.colors = colors;

        cache = Bitmap.createBitmap(screenWidth, screenHeight,
    ...
    public void forceDraw() {
        ...
        for (int i = 0; i < colors.length && i * 4 + 3 < rectCoord.length; i++) {
            picture.setColor(colors[i]);
            picture.setStyle(Paint.Style.FILL);
            canvas.drawRect(rectCoord[i * 4], rectCoord[i * 4 + 1],
                    rectCoord[i * 4 + 2], rectCoord[i * 4 + 3], picture);
        }
